...
Recently, the second round was completed \cite{NISTReport}, and the last round started.

In the last round, the scheme SIKE (Supersingular Isogeny Key Exchange) participates as an alternative candidate.
SIKE is based on the relatively new idea of a Diffie-Hellman-like key exchange using isogeny graphs.
...
In this work, we give an introduction to the mathematical basics and the Supersingular Isogeny Diffie-Hellman (SIDH) protocol, which is the main part of SIKE. 

\section{Elliptic Curves as Varieties}

As the name already indicates, the main ingredient for SIDH are isogenies, so a certain kind of maps between Elliptic Curves.
Hence the structure of Elliptic Curves and isogenies are crucial for the rest of this work.
However, this structure has two parts, and the interaction of them is one of the reasons that make Elliptic Curves an extremely fascinating topic in algebra.
Namely, Elliptic Curves are varieties (i.e. objects studied in Algebraic Geometry), but also have a (more or less) canonical group structure.
In this section, we will consider the geometric structure, and move the study of the group structure to the next section.
...
Similar to the convention of defining Elliptic Curves by dehomogenized polynomials, we will use a notation that does not rely on homogeneous coordinates, as defined in \ref{def:rational_map}.
Continuing this path yields a nice way to work with isogenies, but first we require the coordinate ring of an Elliptic Curve.
...
\begin{proof}
    ...
    To show the existence, it suffices to show that for each $a = (a_1, a_2) \in E_1$ there are polynomials $g_1, g_2, g_3 \in \bar{K}[X, Y]$ such that $g_1/g_3$ and $g_2/g_3$ are representatives of $u$ resp. $v$ and $(g_1(a), g_2(a), g_3(a)) \neq (0, 0, 0)$ with $(g_1(a) : g_2(a) : g_3(a)) \in E_2$.
    These polynomials $g_1, g_2, g_3$ then give a well-defined value for $\phi(a)$.

    As $E_1$ is smooth, we get that the localization $\bar{K}[E_1]_a$ of $\bar{K}[E_1]$ at the maximal ideal $(X - a_1, Y - a_2)$ is a discrete valuation domain (\cite[II.1.1]{arithmetic_elliptic_curves}).
    Hence, $\bar{K}[E_1]_a$ has a uniformizer $t \in \bar{K}[E_1]$. 
    Now let $u = u_1 / u_2$ and $v = v_1 / v_2$ for $u_1, u_2, v_1, v_2 \in \bar{K}[E_1]$.
    Then the claim follows with any representatives $g_1, g_2, g_3 \in \bar{K}[T]$ such that
    \begin{equation*}
        \overline{g_1} = t^{-d} u_1v_2, \quad \overline{g_2} = t^{-d} v_1u_2, \quad \overline{g_3} = t^{-d} v_2u_2 \in \bar{K}[E]_a
    \end{equation*}
    where
    \begin{equation*}
        d = \min \{ \mathrm{val}_t(u_1v_2), \mathrm{val}_t(v_1u_2), \mathrm{val}_t(v_2u_2) \} \in \N
    \end{equation*}
    and $\mathrm{val}_t$ is the valuation is the DVD $\bar{K}[E_1]_a$.

    As we have $F(u, v) = 0 \in \bar{K}(E_1)$, it follows that 
    \begin{equation*}
        F^{\mathrm{hom}}\Bigl( \frac {g_1(X)} {g_3(X)}, \frac {g_2(X)} {g_3(X)}, 1 \Bigr) = g_3^{-1}(X) \ F^{\mathrm{hom}}(g_1(X), g_2(X), g_3(X)) = 0 \in \bar{K}(E_1)
    \end{equation*}
    and thus
    \begin{equation*}
...
Interestingly, it has very deep connections to the geometric properties of Elliptic Curves, which might seem strange at first but gives rise to a huge and complex theory.
...
\end{prop}
\begin{proof}
    Follows directly from Bezout's Theorem.
\end{proof}
...
Note that $+_{\mathrm{poly}}$ is given by polynomials.

\begin{remark}
    \label{prop:sum_morphisms}
    Both the ``translation'' map and the ``scalar multiplication'' map are mor-phisms.
    ...
    These special cases are easily checked by another computation, however.
    Thus, the sum of morphisms is again a morphism. 
    
    It follows that the translation map
    \begin{equation*}
        \tau_Q: E \to E, \quad P \mapsto P + Q
    \end{equation*}
...
\begin{proof}
    ...
    Note that it is also surjective.
    ...
    By a more complicated but not fundamentally different calculation one finds that also
    \begin{equation*}
        N(Y - \mu) = c(\psi_2 - \psi_2(\lambda, \mu)) \quad \text{for some $c \in \bar{K}^*$}
    \end{equation*}
    So
    \begin{equation*}
...
\begin{definition}
    ...
    \begin{equation*}
        E[n] := \{ P \in E(\F_q) \ | \ [n] \cdot a = \O \} \leq E
    \end{equation*}
