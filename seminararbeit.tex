...
This definition is quite technical, as one wants a local definition.
...
As a result, their areas of intersection are again open in the Zariski-topology, and thus already uniquely determine the polynomial.
The result is that all the polynomials that are glued together must be ``equivalent'' according to the above definition.
...
This will be proven in the next section, by using the following result.

\begin{lemma}
    \label{prop:finite_kernel}
    Let $E_1, E_2$ be Elliptic Curves and $\phi: E_1 \to E_2$ a nonconstant isogeny. Then the fiber $\phi^{-1}(\{\O\})$ is finite.
\end{lemma}
\begin{proof}
    As $\phi$ is a morphism, $V := \phi^{-1}(\{\O\})$ is a subvariety of $E_1$. As $E_1$ is irreducible and by assumption $V \neq E_1$, it follows that $V$ is zero-dimensional, so consists of finitely many points.
\end{proof}
...
\begin{proof}
    ...
    \end{equation*}
    This is well-defined, because if $\overline{f} = \overline{g} \in \bar{K}[E']$, then $f - g$ vanishes on $E'$ an thus $f(\psi_1, \psi_2, \psi_3) - g(\psi_1, \psi_2, \psi_3)$ vanishes everywhere, thus is zero.
    Note that
    \begin{equation*}
    ...
    Now we have by Proposition~\ref{prop:dedekind_domain} that the following diagram is commutative
...
With this, a fact mentioned earlier directly follows:
\begin{corollary}
    \label{prop:isogeny_surjective}
    Let $\psi: E_1 \to E_2$ be a nonconstant isogeny between Elliptic Curves. Then $\psi$ is surjective.
\end{corollary}
\begin{proof}
    Assume $\psi$ would not be surjective. By the Main Theorem of Elimination theory, its image $\mathrm{im}\psi$ then is a proper subvariety of $E_2$, and as $E_2$ is irreducible, it must be zero-dimensional.
    Hence $\mathrm{im}\psi$ is finite, but by \ref{prop:finite_kernel} also $\ker\psi$ is finite.
    By the isomorphism theorem, it follows that $E_1$ must be finite, as $E_1/\ker\psi \cong \mathrm{im}\psi$.
    However $K$ is algebraically closed, so $E_1$ is infinite, a contradiction.
\end{proof}
