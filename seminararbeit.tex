...
\begin{proof}
    ...
    \begin{equation*}
        K(E) = \Quot\left(K[x, y] / (y^2 - x^3 - A x - B)\right) \cong \Quot\left(K[X, Z]/(Z - X^3 - A X Z^2 - B Z^3)\right)
    \end{equation*}
    via $x \mapsto X/Z, \ y \mapsto 1/Z$. We identify those two representations.
    
    Now consider the localization
    \begin{equation*}
        R := K[E]_{(0, 0)} := K[E]_{\mathfrak{m}} \ \text{where} \ \mathfrak{m} = \langle x, z \rangle = \{ f \in K[E] \ | \ f(0, 0) = 0 \}
    \end{equation*}
    of the coordinate ring $K[E] = K[X, Z]/(Z - X^3 - A X Z^2 - B Z^3)$ at $(0, 0) \in K^2$.
    ...
    \begin{equation*}
        V_2 := \{ f \in K(E)^* \ | \ \text{valuation of $f$ in $R$} \geq -2 \} \cup \{ 0 \}
    \end{equation*}
